      ...
      for (rep in seq_len(reps)) {
        ...
        for (i in 1:k) {
          if (!is.null(time_var) & !is.null(space_var)) {
            self$instance[[rep]]$test[[i]] = which(data[[space_var]] %in%
              sptfolds$spacefolds[[i]] &&
              data[[time_var]] %in% sptfolds$timefolds[[i]])
            self$instance[[rep]]$train[[i]] = which(!data[[space_var]] %in%
              sptfolds$spacefolds[[i]] &&
            ...
            self$instance[[rep]]$test[[i]] = which(data[[time_var]] %in%
              sptfolds$timefolds[[i]])
            self$instance[[rep]]$train[[i]] = which(!data[[time_var]] %in%
              sptfolds$timefolds[[i]])
          }
        }
      }
