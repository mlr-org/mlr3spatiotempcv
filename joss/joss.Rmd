...
This leads to a high similarity between train and test sets, resulting in "better" but over-optimistic performance estimates in every fold of a CV compared to the spatial CV approach.
...
Results from a SptCV are not unbiased but more close in reflecting the actual performance of the learner on unknown datasets.
...
The performance of a simple classification tree (`"classif.rpart"`) is evaluated on a random repeated cross-validation (`"repeated_cv"`) with four folds and two repetitions.
