...
In this situation it would therefore seem rather foolish to train a model on a simple random subsample of grid cells, and test it on the remaining data, i.e. using other grid cells from the same fields, as if we wanted to predict within a field.
...
The earliest and most comprehensive of these implementations is the \pkg{sperrorest} package [@brenning2012], which provides an extensible framework and includes predefined resampling strategies based on, for example, blocking, grouping and buffering.
...
...
<!-- FIXME: @alex: do you want to see more into here? -->
...
By introducing more distance between observations, the influence of STAC should be reduced.
Buffering is mostly applied in the coordinate space due to the scattered appearance of observations in this space.
\pkg{mlr3spatiotempcv} supports two "buffering" implementations: (1) "Spatial buffering" from package \pkg{blockCV} and (2) "Leave-one-disc-out cross-validation" from \pkg{sperrorest}.
...
<!--- P: By default the buffering implementation of blockCV behaves like a normal LOOCV.
It has additional features specific for SDM (presence-background data) but these are not active by default.
That is why I did not add `sperrorest::partition_loo()` because `blockCV::buffering()` does the same + has support for SDM. -->


<!-- A: This paragraph is a general description of the approach, not a description of unique characteristics of this implementation. General approach of buffering is (and should be) described in the introdoctory paragraph of the Buffering section.
...
The implementation in the \pkg{blockCV} package comes with optional support for presence/absence data handling, as often used in the field of species distribution modeling (SDM) [@hijmans2013].
...
<!--- P: See my comment above, it might be that this is a misunderstanding. If not (or anyhow), we might want to clarify this more than to prevent such misunderstandings. -->
...
<!--- P: Validated here means: the existence of the presence observations is validated, the background observations are artificial. This is a common procedure in SDM.
The blockCV buffering integration comes with support for presence/background handling which is important for SDM.
I think this should be explained shortly then - which is what this section is about. -->
...
```{r buffer, fig.cap="Visualization of the 'Spatial buffering' method from package `blockCV`.", out.width="50%", fig.pos="ht"}
...
<!--- P: Because this is one method within the "buffering" category and so far my approach was to label these as "<method name> (<pkg>)".
Apparently this is sub-optimal from your point of view - how can we do better?
Also the visual appearance of level 4 headings in the PDF is sub-optimal - maybe we should do this someway different.
I've fixed the level 4 headers, they appear as proper formatted headers.
I also opened an issue on GH to add some introductory information to each method category. -->
...
```{r disc, fig.cap="Visualization of 'Leave-one-disc-out' method.", out.width="50%"}
...
The "Spatial CV" method after [@russ2010; @brenning2012] and implemented in package \pkg{sperrorest} uses the coordinates of all observations to create clusters in the spatial domain.
$k$-means clustering has been proposed as a clustering method.
To avoid confusion with other spatial partitioning methods, we refer to this as coordinate-based clustering, or as `spcv_coords` in \pkg{mlr3spatiotempcv}.
...
Nevertheless, despite the random selection of initial cluster centers, repeated partitionings may in some cases be nearly identical. Also, $k$-means clustering may be less suitable for data sets with existing clusters of points and/or with distant isolated sample locations.

```{r coords, fig.cap="Method 'Spatial CV' showing k-means clustering based on coordinates.", out.width="50%", fig.pos="ht"}
...
The idea to follow a such an inherited grouping structure of the data is also referred to as "target-oriented validation" [@cast].
Even though flexibility is limited due to the inherited grouping structure within the data, some implementations allow to specify the resulting number of folds by omitting parts of the dataset.

\pkg{mlr3spatiotempcv} supports two "grouping" implementations: (1) The "Leave-location-and-time-out" family from package \pkg{CAST} and (2) "Custom Resampling" from \pkg{mlr3}.
...
The 3D visualization (space + time) has been implemented via \pkg{plotly}.
...
Support for user-defined partitioning strategies is built into \pkg{mlr3} directly since version FIXME.
Labelled as "Custom CV", users are able to supply an external factor vector with the same length as number of observations.
Alternatively to supplying an external factor vector, the name of a feature within the task can be used.

The following example is taken from `sperrorest::partition_factor()`.

<!-- FIXME: eval=TRUE when available upstream im mlr3 -->

```{r custom-cv,  fig.cap="Environmental blocking using a single predictor.", out.width="50%", fig.pos="ht", eval=FALSE}
...
resampling_custom$instantiate(task, split = zclass)
```
...
The specification of such is not always easy because the selection of such a variable also implies that the selected groups inherit substantial autocorrelation and differ substantially from other groups.
...
We believe that making resampling methods available to widely used modeling frameworks is more beneficial than implementing such methods standalone in an R package.
The latter often require special structured objects as inputs and cannot directly be re-used in subsequent modeling steps of user workflows.
