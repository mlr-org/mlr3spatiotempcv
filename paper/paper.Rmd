...
Block-level resampling from package \pkg{blockCV} supports both random and systematic resampling of square blocks (`method = "spcv_block"` with argument `selection = "random"` and `"systematic"`, respectively; Figure&nbpsp;~\@ref(fig:block-random) and \@ref(fig:block-systematic)).
The implementation in \pkg{blockCV} has specific options that are designed for modeling presence-only data, which is a typical situation in species distribution modeling.
