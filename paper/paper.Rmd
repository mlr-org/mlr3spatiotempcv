...
abstract: >
  ...
  This simplifies the step of integrating package specific syntax into an overarching machine learning pipeline, making it easier for users to apply a variety of different spatiotemporal resampling methods.
  The mlr3spatiotempcv package refrains from making suggestions which resampling methods users should apply as this decision mainly depends on the prediction purpose of the fitted model.
...
In this specific situation it would therefore seem rather unwise to train a model on a simple random subsample of grid cells, and test it on the remaining data, i.e. using other grid cells from the same fields, as if we wanted to predict within a field.
...
We would like to emphasize the conceptual distinction between CV *at the group (or block) level*, referring to the latter scenario, from *custom (or grouped)* CV, where the blocks or groups themselves define the CV partitions.
...
\begin{table}[h]
  \centering
  \footnotesize
  \caption[t]{Spatiotemporal resampling methods, ordered alphabetically by class name (column 'mlr3 Notation').
    Column count refers to the number of studies which were found to have used this resampling technique until 2021-05.
...
This kind of blocking might be of special interest for SDM studies as well as the possibility to supply a user-defined polygon via argument `rasterLayer` with predefined blocking zones.
...
In other words, accounting for one dimension as in Fig. \@ref(fig:lto) or Fig. \@ref(fig:llo), by only having one time step or location in either test or training set, would have no effect on the respective other.
Thus, the removal of observation can be seen as a proxied buffering in multiple dimensions as shown in Fig. \@ref(fig:llto).
...
This partitioning concept is also ften referred to as "grouped resampling" and should not be mistaken with "group-level resampling" (section \@ref(sec:group-level)), which inherits additional aggregation of groups at the partition level.
...
- Are some of the predictor variables under consideration known to be highly affected by spatiotemporal autocorrelation such as temperature or precipitation?

Based on these criteria users should choose a matching partitioning method that is either more restrictive (by discarding observations for fold creation) or more liberal (by using all observations and eventually ignoring natural grouping patterns).
<!--JM: what do you mean by "using all observations"?-->
...
Last, a note on best practices for the example shown in \@ref(sec:case-study):
...
Rectangular partitioning can be problematic for irregularly shaped study areas as it might result in certain partitions containing substantially less observations than others.
...
Choosing an appropriate buffer size is a difficult task.
In any case, one should refrain oneself from selecting a value based on subjective notions with regard to study area characteristics.
...
On the upside, developers can benefit from existing package structures and orient themselves on how previous methods were implemented.
