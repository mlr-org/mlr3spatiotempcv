...
abstract: >
  ...
  This contribution reviews the state-of-the-art in spatial and spatiotemporal CV, and introduces the R package [mlr3spatiotempcv](https://mlr3spatiotempcv.mlr-org.com) as an extension package of the machine-learning framework [mlr3](https://mlr3.mlr-org.com), demonstrating its use in a step-by-step example.
  ...
  This simplifies the the integration of package-specific syntax into an overarching machine-learning pipeline, making it easier for users to choose from a variety of spatiotemporal resampling methods.
    The users' decision to use a specfic resampling method should depend on the predictive task at hand, the autocorrelation within the data, and the spatial structure of the sampling design or geographic objects being studied.
keywords:
...
The `"spcv_block"` method from package \pkg{blockCV} supports both random and systematic resampling of square blocks (`rsmp("spcv_block")` with argument `selection = "random"` and `"systematic"`, respectively; Figures \@ref(fig:block-random) and \@ref(fig:block-systematic)).
...
Block sizes (square) (in meters) are determined by the `range` argument.
...
It should be noted that rectangular partitioning can be problematic in irregularly shaped study areas as in Figure \@ref(fig:block-random) where some of the resulting partitions may contain substantially fewer observations than others.
...
In the mathematical sense, however, this buffering is not based on a valid metric (or distance function) in three-dimensional space (2D + time) as neither the identity of detectability nor the triangle inequality are satisfied by the underlying combined 'distance' measure.
