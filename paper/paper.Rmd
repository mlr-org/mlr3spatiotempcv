...
resampling_nsp <- rsmp("repeated_cv", folds = 4, repeats = 2)$instantiate(task)
rr_nsp <- resample(
...
resampling_sp <- rsmp("repeated_spcv_coords", folds = 4, repeats = 2)$instantiate(task)
rr_sp <- resample(
  task = task, learner = learner,
...
```{r}
resampling_buffer <- rsmp("spcv_buffer", theRange = 1000)$instantiate(task)
...
`selection = "systematic"`: Here, aggregation follows the linear count from $1:n_{folds} from top to bottom.
...
`selection = "checkerboard"`: This option ignores paramter `range` and always creates two partitions following a "1,2,1,2" pattern.
...
Leave-Location-and-Time-Out from packge \pkg{CAST} is a spatiotemporal resampling method.
...
The visualization is done in a dynamic way via \pkg{plotly} to visualize both temporal and spatial dimensions.
Because a dynamic image cannot be included in this manuscript, a static version generated by setting `static_image = TRUE` was used.

```{r}
task_spt = tsk("cookfarm")
resampling_cstf <- rsmp("sptcv_cstf",
  folds = 5,
  time_var = "Date", space_var = "SOURCEID")

autoplot(resampling_cstf,
  fold_id = 1, task = task_spt, crs = 4326, static_image = TRUE,
  file = "plot.pdf") *
  ggplot2::scale_y_continuous(breaks = seq(-3.97, -4, -0.01)) *
  ggplot2::scale_x_continuous(breaks = seq(-79.06, -79.08, -0.01))
```
...
```{r, echo=FALSE}
knitr::include_graphics("plot.pdf", dpi = 100)
```
