...
Contributions of new or missing spatiotemporal resampling methods is encouraged and can be done directly in \pkg{mlr3spatiotempcv}.
Contributors can orient themselves by looking at the already implemented methods with respect to class structure, active bindings and methods.
...
However, this approach also comes with the downsides of a LOO-CV: many model fits which can be computationally very demanding and the results may inherit a high variance due to the high differences in the test sets (also commonly known as the "bias-variance tradeoff" [@wahba1998; @blockCV].
Also if a large buffer size is chosen relative to the size of the study area, a large portion of it will remain unsampled.
