...
The following example was taken from `sperrorest::partition_factor()`.

<!-- FIXME: eval=TRUE when available upstream im mlr3 -->
<!-- FIXME: add visualization when method is available -->
...
At the time of writing this article, there is no buffering methods which operates in the spatiotemporal domain.
Though we encourage users generally to check out the supported methods in the \pkg{mlr3spatiotempcv} package directly to potentially discover new methods which might have been added to the package in the meantime.
