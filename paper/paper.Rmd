...
Additionally, the user might supply a numeric value in meters via argument `range` which will ensure a quadratic block pattern across the study area.
...
Alternatively, arguments `rows` and `cols` can be specified instead of `range`.
...
To set up the task `"ecuador"` manually, we would have to proceed as follows:

```{r create-task}
# equivalent to 'tsk("ecuador")':
...
Spatial CV, in contrast, provides a better measure of a model's ability to generalize from the training sample --- in the Ecuador case study, from the hillslopes and historical landslides in the training sample.
It is also expected that spatial CV results better reflect the performance of a model on new, spatially separate data.
The magnitude of this difference may depend on the dataset, the strength on spatial or spatiotemporal autocorrelation, and the learner itself.
