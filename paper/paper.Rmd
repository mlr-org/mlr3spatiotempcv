...
```{r disc-eval, fig.cap='Visualization of one training set / test set combination generated with the leave-one-disc-out method from package sperrorest (method "spcv\\_disc" in mlr3spatiotempcv). The disc has a radius of 300 m and is surrounded by a 400-m buffer.', out.width="40%", echo=FALSE,fig.pos="hb"}
library("mlr3")
library("mlr3spatiotempcv")
task = tsk("ecuador")
rsmp_disc = rsmp("spcv_disc", folds = 100, radius = 300L, buffer = 400L)
...
rsmp_coords = rsmp("spcv_coords", folds = 5)$instantiate(task)
...
```{r custom-cv, fig.cap='Leave-one-level-out (custom) resampling from package mlr3 (method "custom\\_cv"). A factor variable is used to define the partitions. Each color represents one partition.', out.width="40%", fig.pos="ht", echo=FALSE}
breaks = quantile(task$data()$dem, seq(0, 1, length = 6))
zclass = cut(task$data()$dem, breaks, include.lowest = TRUE)
...
plot_env_single = autoplot(rsmp_env, size = 0.8, fold_id = 1, task = task)

plot_env_multi = autoplot(rsmp_env_multi, size = 0.8, fold_id = 1, task = task)
...
```{r env-1-eval, fig.cap='Environmental leave-one-block-out CV from package blockCV using one (left, "distdeforest") and two (right, "distdeforest" and "slope") predictors to define blocks in the feature space.', out.width="100%", fig.pos="ht", echo=FALSE}
