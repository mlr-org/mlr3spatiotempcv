...
To reduce first-level dependencies, some methods were reimplemented instead of imported from upstream packages.
...
"Buffering" refers to the idea of removing observations between test and training set by drawing a cirular buffer around one or more observations [@rest2014, @brenning2012, @blockCV].
...
<!--- P: Validated here means: the existence of the presence observations is validated, the background observations are artifical. This is a common procedure in SDM.
The blockCV buffering integration comes with support for presence/background handling which is important for SDM.
I think this should be explaind shortly then - which is what this section is about. -->
...
In the visualization of the first fold in Fig. \@ref(fig:buffer) a buffer zone of 1000 m was used without any custom presence/absence treament of the data.
...
Apparently this is suboptimal from your point of view - how can we do better?
Also the visual appearance of level 4 headings in the PDF is suboptimal - maybe we should do this someway different.
I've fixed the leavel 4 headers, they appaer as proper formatted headers.
...
Also a seed is set for deterministic executiona and the \pkg{mlr3spatiotempcv} built-in task `"ecuador"` is loaded via `tsk()`.
