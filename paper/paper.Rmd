...
For example, ...
<!--JM: maybe refer back to the crop example of the intro to make the point better comprehensible than just giving the mere theory-->

There are various variables influencing the partitioning method choice for an environmental dataset:

- Do natural groupings exists in the data? <!--Example-->
...
- Are some of the predictor variables under consideration known to be highly affected by spatiotemporal autocorrelation such as ... <!-- pls add an example -->?
...
On the upside, `"spcv_coords"` works well with irregularly shaped study areas due to the ability to create partitions which adjust well to the given study area shape.
...
If a large buffer size is chosen relatively to the overall study area size, a large portion of it will not be sampled.
Choosing an appropriate buffer size is not easy and it should be taken care not to select a value based on subjective interpretation of the study area characteristics.
