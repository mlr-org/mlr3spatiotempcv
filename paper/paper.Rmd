...
The terminology "block" used in this categorization and in the manuscript in general refers to any kind of regular and irregularly shaped grouping of observations which divides the study area into different zones [russ2010; @bahn2012; @wenger2012].
This is different to the meaning of rectangular-only partitioning within some specific resampling methods [@blockCV].
...
In the visualization of the first fold in Figure&nbsp;\@ref(fig:buffer-eval) a buffer zone of 1000 m was used without any custom presence/absence treatment of the data.
...
The `"cv"` resampling strategy performs random, non-spatial partitioning.
To use it in a spatiotemporal context, one can set the "group" column role in a mlr3 Task object for specific variable(s).
This tells the resampling strategy to honor these grouping(s) during partitioning.
In the final folds, these groups of observations will either belong to the test or training set and by this create possibly spatiotemporal resampling folds.

The idea is to make use of inherent grouping structure in the observations or the sampling design in space and/or in time to determine partitions.
In contrast to geometric blocking or clustering, the spatial or temporal location is not used explicitly, but additional metadata is exploited such as the membership in a specific agricultural management unit [@brenning2006], or in a site in hierarchical sampling [@kasurak2011].
In \pkg{mlr} this concept was called "blocking".

In the following example we create a custom grouping of eight classes using a kmeans clustering.
...
To showcase the honoring of the groups a uneven amount of folds (with respect to the number of groups) is requested.
The test set in the first and second fold is composed out of three groups while the third fold is only made up from two groups.
...
Next, the created resampling object `rsmp_nsp` is passed to function `resample()` together with the task and learner objects created earlier to execute the model assessment.
...
Finally, we visualize the partitions that were used during performance estimation making use of the generic `autoplot()` function in package \pkg{mlr3spatiotempcv} (Figure&nbsp;\@ref(fig:vis-spcv-eval)).
...
The non-spatial CV estimate of AUC (0.76) is around 0.13 higher compared to the spatial CV estimate of 0.64.
The higher non-spatial CV value results from over-optimism in the estimation with non-spatial CV and does imply better performance.
...
It is also expected that spatial CV results reflect a more honest performance of a model on new, spatially separate data.
...
This may, for example, involve a reduced maximum tree depth in the Ecuador case study, since this parameter leads to stronger generalization and reduced overfitting.
...
Although the case study example in section \@ref(sec:case-study) used the `"spcv_coords"` method, this should not give the impression that this method is the best or only method suitable for this example task.
...
Hence, this method is a valid choice for the given dataset.
...
The prediction purpose is the model's ability for which areas or zones it actually can make predictions for.
