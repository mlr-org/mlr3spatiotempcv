...
abstract: >
  ...
  This contribution reviews the state-of-the-art in spatial and spatiotemporal CV, and introduces the R package [mlr3spatiotempcv](https://mlr3spatiotempcv.mlr-org.com) as an extension package of the machine-learning framework [mlr3](https://mlr3.mlr-org.com).
  Currently various R packages implementing different spatiotemporal partitioning strategies exist: blockCV, CAST, skmeans and sperrorest.
...
The earliest and most comprehensive of these implementations is the \pkg{sperrorest} R package [@brenning2012], which provides an extensible framework and includes predefined resampling strategies based on geometric blocking, clustering, and buffering.
...
The purpose of this article is to give an overview of the methods implemented in the R package \pkg{mlr3spatiotempcv}.
...
Approaches based on *spatial blocking* (or somtimes calld *grouping*) require either the construction of spatial zones, or the use of pre-existing spatial structures in the data.
...
`autoplot()` is \pkg{ggplot2}-based and uses [ggplot2](https://ggplot2.tidyverse.org) [@ggplot2] in two-dimensional geographic space and [plotly](https://github.com/ropensci/plotly) [@plotly] in the three dimensions case, i.e., geographic space plus time.
...
\begin{table}[h]
  ...
  \caption[t]{Spatiotemporal resampling methods available in the mlr3 ecosystem.
    The "Name" column shows the mlr3 method name as found in the \texttt{mlr3::mlr\_resamplings} dictionary.
...
The test sets in the first and second folds are therefore composed out of three groups while the third one holds two groups.
...
```{r cv-non-eval-2, fig.cap='Cross-Validation at the block level including predefined groups from package mlr3 (method "cv"). A factor variable (named "group") was used to define the grouping. Each class is either assigned to the test or training set.', out.width="40%", fig.pos="ht", eval=FALSE}
...
In this section the `cookfarm` dataset is used as an example because it comes with a temporal dimension via variable "Date".
...
The heavy lifiting of the 3D visualization (i.e. 2D + time) option is done via package \pkg{plotly}.
...
```{r llo, echo=FALSE, out.width="70%", fig.cap='Birds-eye view of "leave-location-out" CV from package CAST (method "sptcv\\_cstf" and argument space\\_var = "SOURCEID").', fig.pos="ht"}
...
In \pkg{mlr3}, machine-learning tasks with their respective dataset and response variable are represented by objects of class `Task`. \pkg{mlr3spatiotempcv}'s spatial and spatiotemporal machine-learning tasks are also derived from this superclass.
...
Even though there is and most likely will be no definitive answer to this question, we would like to give some guidance in this section to help finding an appropriate method.
