...
While sophisticated mathematical frameworks have long been developed in spatial statistics to characterize predictive uncertainties under well-defined mathematical assumptions such as intrinsic stationarity [e.g., cressie2015], computational estimation procedures have only been proposed more recently to assess predictive performances of spatial and spatiotemporal prediction models [@brenning2005; @sperrorest; @pohjankukka2017; @roberts2017].
...
Other more recent implementations include \pkg{blockCV} for (what for?) [ref @blockCV; @rest2014] and \pkg{CAST} for spatiotemporal prediction tasks in a \pkg{caret} framework [@cast; @meyer2018a]. 

Thus, \pkg{mlr3spatiotempcv} implements for the first time a comprehensive state-of-the-art compilation of spatial and spatiotemporal partitioning schemes that is well-integrated into a major machine-learning framework in R, the \pkg{mlr3} ecosystem [@mlr3]. 
...
With the increased awareness of the importance of spatial and spatiotemporal resampling strategies and the growing popularity of R in environmental modelling and geocomputation, it is important to equip machine-learning frameworks such as \pkg{mlr} with suitable algorithms. 
...
Resampling objects in \pkg{mlr3spatiotmpcv} inherit from and can be created from established object classes for geospatial data in R, including simple features (add a general reference, maybe Lovelace), which facilitates its integration into domain-specific workflows in the geospatial sciences. This includes support for geographic coordinate reference systems (CRS) and their respective distance measures in projected planar space or using great circle distances on the unprojected ellipsoid (corrrect???). Non-spatial data tables with coordinates as variables are also supported. If no projection is specified, World Geodetic System 1984 (WGS84, EPSG: 4326) will be assumed.

Partitioning objects in \pkg{mlr3spatiotempcv} are equipped with generic `plot()` and `autoplot()` methods that visualize the created groups. `autoplot()` is \pkg{ggplot2}-based and uses [ggplot2](https://ggplot2.tidyverse.org) [@ggplot2] in two-dimensional geographic space and [plotly](https://github.com/ropensci/plotly) [@plotly] in three dimensions, i.e. geographic space plus time.
