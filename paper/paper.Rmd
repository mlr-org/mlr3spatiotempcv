...
Buffering mostly happens in the coordinate space due to the scattered appearance of observations in this space.
...
<!--- FIXME: Why refer to the blockCV package? This is a general method, as described (or at least outlined) above. If code from blockCV was used, when this should be acknowledged, but otherwise I don't understand why this method is "from" the blockCV package. -->

<!--- P: This addresses specifically the "buffering" function in the blockCV package which falls into the "buffering" category, the same applies to "spatial disc" from {sperrorest}.
Is this a potential misunderstanding with the overarching level 3 header "Buffering"? -->
...
However, by default, the method considers all points for training/test set creation and does not apply any specific SDM adjustments to the data.
