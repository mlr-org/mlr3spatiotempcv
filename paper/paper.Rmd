...
Although alternatives such as the bootstrap exist and have some advantages [@efron1983; @milligan2000], cross-validation is a particularly well-established, easy-to-implement algorithm for *model assessment* of supervised machine-learning models [@efron1983 and next section] and *model selection* [@arlot2010].
...
The earliest of these implementations is the \pkg{sperrorest} package [@brenning2012], which has, however, not been integrated into established machine-learning frameworks such as \pkg{mlr} or \pkg{caret}, and lacks support of temporal prediction tasks.
...
This is referred to as the *actual* or *conditional* performance measure as it is conditional on $L$ [@milligan2000].
...
The choice of the performance measure is equally critical as the choice of the estimation procedure, but it is beyond the scope of this contribution to discuss performance metrics for regression and classification [see e.g., @milligan2000 for classification, and ref?? for regression tasks].
...
\begin{table}[h]
  ...
  \begingroup
    \begin{tabular}{lllll}
      ...
      \toprule
      \multirow{6}{*}{Spatial}        & Spatial Buffering           & \specialcell{\pkg{blockCV} \\ \pkg{sperrorest}}    & \specialcell{ \texttt{"spcv\_buffer"} \\  \texttt{"spcv\_disc"} \tablefootnote{\label{endicott2017}\cite{endicott2017}} }  & \tablefootnote{\label{ploton2020}\cite{ploton2020}}  \tablefootnote{\label{diesing2020}\cite{diesing2020}}      \\
      \cmidrule{2-5}
                                      & Spatial Blocking            & \specialcell{\pkg{blockCV} \\ \pkg{sperrorest}}  & \specialcell{  \texttt{"spcv\_block"} \\  \texttt{"spcv\_tiles"} \tablefootnote{\label{bebber2017}\cite{bebber2017}} \tablefootnote{\label{brenning2015}\cite{brenning2015}} \tablefootnote{\label{zurell2020}\cite{zurell2020}} }
                                        & \tablefootnote{\label{jensen2021}\cite{jensen2021}} \tablefootnote{\label{escobar2021}\cite{escobar2021}} \tablefootnote{\label{stewart2021}\cite{stewart2021}} \tablefootnote{\label{cerasoli2021}\cite{cerasoli2021}}        \\
  \cmidrule{2-5}
                                      & Spatial Clustering          & \pkg{sperrorest} & \texttt{"spcv\_coords"} & \tablefootnote{\label{note1}\cite{brenning2012}} \tablefootnote{\label{morera2021}\cite{morera2021}} \tablefootnote{\label{geiss2017}\cite{geiss2017}} \tablefootnote{\label{wu2020}\cite{wu2020}}      \\
                                      \cmidrule{2-5}
       & Leave-Location-Out & \specialcell{\pkg{CAST} \\ \pkg{mlr3}}       & \specialcell{ \texttt{"sptcv\_cstf('space\_var')"} \\ \texttt{"FIXME"}},  & \footref{note1}       \\
      \midrule
      \multirow{2}{*}{Spatiotemporal} & Leave-Location-and-Time-Out & \pkg{CAST}       &  \texttt{"sptcv\_cstf"}  & \\
                                      & Spatiotemporal Clustering   & \pkg{skmeans}    &  \texttt{"sptcv\_cluto"} &        \\
      \midrule
      Feature space                   & Environmental Blocking      & \pkg{blockCV}    &  \texttt{"spcv\_env"}    & \footref{morera2021}     \\
