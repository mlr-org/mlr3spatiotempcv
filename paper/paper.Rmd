...
# List of spatiotemporal resampling methods

Table \@ref(tab:sptcv-methods) lists all available spatiotemporal resampling methods available in \pkg{mlr3spatiotempcv}.
Column `Package` denotes the upstream package from which the method was incorporated.
...
## A critical evaluation of all methods

Even though all methods have their pros and cons and no universal method exists, this chapter should provide a critial view on the limitations of each method.

<!-- spcv coords: mention generic name -->

<!-- sptcv LLTO: discards many observations -->
<!-- sptcv LLO: requires additional variable to specify groupings -->
<!-- sptcv LTO: ignores spatial autocor entirely -->
