...
Compared to other existing machine-learning frameworks in R (e.g., \pkg{caret} [@kuhn2020] or \pkg{tidymodels} [@tidymodels]), \pkg{mlr3} is the only one that provides a dedicated object-oriented framework for spatial and spatiotemporal resampling methods.
...
\begin{table}[ht]
  ...
  \begingroup
    \begin{tabular}{lllll}
      ...
      \midrule
      \multirow{5}{*}{\specialcell{CV at the \\ block level}} &  geometric: rectangular & \texttt{"spcv\_block"}  & \pkg{blockCV} (28)      & \specialcell{\cite{jensen2021} \\ \cite{escobar2021} \\ \cite{stewart2021}} \\
  \cmidrule{2-5}
                                      &  custom & \texttt{"cv"} with grouping  & \pkg{mlr3} (0)  &  -  \\
  \cmidrule{2-5}
                                      &  clustering in feature space & \texttt{"spcv\_env"}* & \pkg{blockCV} (1)   &  \cite{morera2021}  \\
  \cmidrule{2-5}
                                      &  clustering of coordinates + aggregation & \texttt{"spcv\_knndm"}* & \pkg{CAST} (1)   &  \cite{ludwig2023}  \\
      \midrule
