...
% this is required to not expand the authors in the table citations on first use
...
\begin{table}[ht]
  ...
  \cmidrule{2-5}
      ...
      \midrule
      \multirow{3}{*}{\specialcell{Spatiotemp. \\ CV}} &  custom & \texttt{"sptcv\_cstf"} & \pkg{CAST} (6)      & \specialcell{\cite{gao2019} \\ \cite{reitz2021} \\ \cite{egli2020}} \\
                                                  \cmidrule{2-5}
                                      &  clustering: custom & \texttt{"sptcv\_cluto"}  & \pkg{skmeans} (0)    & - \\
                                      \bottomrule
    \end{tabular}
  \endgroup
...
plot_env_single = autoplot(rsmp_env, size = 0.3, fold_id = 1, task = task) +

  plot_env_multi = autoplot(rsmp_env_multi, size = 0.3, fold_id = 1, task = task)
...
```{r llo, echo=FALSE, out.width="70%", fig.cap='Birds-eye view of `"leave-location-out"` CV from package \\pkg{CAST} (method `"sptcv\\_cstf"` and column role `"space" = "SOURCEID"`).', fig.pos="ht"}
