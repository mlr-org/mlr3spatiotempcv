...
```{r vis-spcv-eval, fig.cap="Spatial leave-one-block-out partitioning using coordinate-based clustering to create roughly equally sized polygonal blocks. Only the first two folds of the first repetition are shown.", fig.pos="ht", out.width="100%", echo=FALSE}
autoplot(rsmp_sp, task, fold_id = 1:2, size = 0.3) *
  ggplot2::scale_y_continuous(breaks = seq(-3.97, -4, -0.02)) *
  ggplot2::scale_x_continuous(breaks = seq(-79.06, -79.08, -0.02)) *
  ggplot2::theme(plot.title = ggtext::element_textbox(size = 8),
