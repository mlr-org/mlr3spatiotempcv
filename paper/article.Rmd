...
abstract: >
  ...
  This contribution reviews the state-of-the-art in spatial and spatiotemporal CV, and introduces the \proglang{R} package \pkg{mlr3spatiotempcv} as an extension package of the machine-learning framework \pkg{mlr3}.
...
However, neither of these have been integrated into established machine-learning frameworks such as \pkg{mlr}/\pkg{mlr3} [@mlr3] or \pkg{caret}/\pkg{tidymodels} [@kuhn2020], and all of them lack support for temporal prediction tasks.
...
One limitation of all these packages is the sole focus on model assessment, while the proposed implementation within the \pkg{mlr3} framework also offers seamless integration into model selection and provides parallel execution and enhanced logging abilities.
...
\begin{table}[ht]
  ...
  \begingroup
    \begin{tabular}{lllll}
      \\
      Type                            & Sub-type & Name                        & \proglang{R} package     & \# pub. \\
      \toprule
  ...
  \cmidrule{2-5}
    ...
    \end{tabular}
  \endgroup
  \caption[b]{Spatiotemporal resampling methods available in the \pkg{mlr3} ecosystem.
    ...
    For each method, up to three randomly selected references were added to the table as footnotes.}
...
### Spatial leave-one-out with buffer
...
```{r buffer-eval, fig.cap='Visualization of the spatial buffering method from package \\pkg{blockCV} (method "spcv\\_buffer" in \\pkg{mlr3spatiotempcv}). The buffer distance is 1000 m.', out.width="40%", fig.pos="ht",echo=FALSE}
library("mlr3")
library("mlr3spatiotempcv")
task = tsk("ecuador")
rsmp_buffer = rsmp("spcv_buffer", theRange = 1000)
...
### Leave-one-disc-out with optional buffer
...
```{r disc-eval, fig.cap='Visualization of one training set / test set combination generated with the leave-one-disc-out method from package \\pkg{sperrorest} (method `"spcv\\_disc"` in \\pkg{mlr3spatiotempcv}). The disc has a radius of 300 m and is surrounded by a 400-m buffer.', out.width="40%", echo=FALSE,fig.pos="ht"}
library("mlr3")
library("mlr3spatiotempcv")
...
### Clustering-based: using coordinates
...
```{r coords-eval, fig.cap='Leave-one-block-out CV based on $k$-means clustering of the coordinates as implemented in package \\pkg{sperrorest} (method `"spcv\\_coords"` in \\pkg{mlr3spatiotempcv}).', out.width="40%", fig.pos="ht", echo=FALSE}
rsmp_coords = rsmp("spcv_coords", folds = 5)
...
### Geometric: using rectangular blocks
...
```{r tile, fig.cap='Leave-one-block-out resampling from package \\pkg{sperrorest} (method `"spcv\\_tiles"` in package \\pkg{mlr3spatiotempcv} with argument `nsplit = c(3,4)` indicating the number of rows and columns).', out.width="40%", fig.pos="ht", echo=FALSE}
rsmp_tiles = rsmp("spcv_tiles", nsplit = c(3L, 4L))
...
<!-- FIXME: -->
### Custom: "customcv" in \pkg{mlr3} {short-title="Custom: customcv in mlr3" #sec:custom-cv}
<!-- {#sec:custom-cv} -->
<!-- \subsubsection[Custom: \code{"customcv"} in mlr3]{Custom: \code{"custom\_cv"} in \pkg{mlr3}} -->
...
```{r custom-cv, fig.cap='Leave-one-level-out (custom) resampling from package \\pkg{mlr3} (method `"custom\\_cv"`). A factor variable is used to define all partitions.', out.width="40%", fig.pos="ht", echo=FALSE}
breaks = quantile(task$data()$dem, seq(0, 1, length = 6))
zclass = cut(task$data()$dem, breaks, include.lowest = TRUE)
...
The `"spcv_block"` method from package \pkg{blockCV} supports both random and systematic resampling of square blocks with argument `selection = "random"` and `"systematic"`, respectively; (see figures \ref{fig:block-random} and \ref{fig:block-systematic}).
...
```{r cv-non-eval-2, fig.cap='Cross-Validation at the block level including predefined groups from package \\pkg{mlr3} (method `"cv"`). A factor variable (named "group") was used to define the grouping. Each class is either assigned to the test or training set.', out.width="40%", fig.pos="ht", eval=FALSE}
...
```{r cv-eval-2, fig.cap='Cross-Validation at the block level including predefined groups from package \\pkg{mlr3} (method `"cv"`). A factor variable is used to define the grouping. Each class is either assigned to the test or training set.', out.width="40%", fig.pos="ht", echo=FALSE}
autoplot(rsmp_cv_group, size = 0.8, task = task_cv, fold_id = 1) *
  ggplot2::scale_y_continuous(breaks = seq(-3.97, -4, -0.01)) *
  ggplot2::scale_x_continuous(breaks = seq(-79.06, -79.08, -0.01))
```

### Clustering: using feature-based clustering
...
In the `cookfarm` example dataset, the `Date` variable was reduced to five unique levels for better visualization, and then used to create a spatiotemporal regression task in \pkg{mlr3spatiotempcv} (Figure \ref{fig:lto}).
...
```{r llo, echo=FALSE, out.width="70%", fig.cap='Birds-eye view of `"leave-location-out"` CV from package \\pkg{CAST} (method `"sptcv\\_cstf"` and argument `space\\_var = "SOURCEID"`).', fig.pos="ht"}
...
```{r llto, echo=FALSE, out.width="70%", fig.cap='Perspective plot of `"leave-location-and-time-out"` CV from package \\pkg{CAST} (method `"sptcv\\_cstf"` and arguments `time\\_var = "Date"` and `space\\_var = "SOURCEID"`). The grey points are excluded from both the training and the test set in this fold.', fig.pos="h"}
...
Although the case study example in section \ref{sec:case-study} used the `"spcv_coords"` method for coordinate-based clustering, this should not give the impression that this method is the only method suitable for this example task.
