...
The following plots are based on a three fold partitioning of the [ecuador]() example task (expect method `p_buffer` which is LOO-CV).
...
# plot
p_block + p_coords + p_env + p_disc + p_buffer +
  plot_layout(ncol = 3) +
